...
;(function($){
	...
	$.fn.scrollabletab = function(options){
		...
		return this.each(function(){
			...
			//Wrap inner items			
			if($.browser.msie!=undefined && $.browser.version=="6.0"){
				$tabs.css({'padding':2, 'position':'relative','overflow-x':'scroll'});
				$tabs.wrap('<div id="stTabswrapper" class="stTabsMainWrapper" style="position:relative;overflow:hidden;width:870px"/>').find('.ui-tabs-nav').css('overflow','hidden').wrapInner('<div class="stTabsInnerWrapper" style="overflow:hidden; position: relative; width:30000px"><span class="stWidthChecker"/>&nbsp;&nbsp;&nbsp;&nbsp;</div>');
			}else{
				$tabs.css({'padding':2, 'position':'relative','overflow':'hidden','width':'870'});
				$tabs.wrap('<div id="stTabswrapper" class="stTabsMainWrapper" style="position:relative;"/>').find('.ui-tabs-nav').css('overflow','hidden').wrapInner('<div class="stTabsInnerWrapper" style="position: relative; width:30000px"><span class="stWidthChecker"/></div>');
			}			
			var $widthChecker = $tabs.find('.stWidthChecker'),
				$itemContainer = $tabs.find('.stTabsInnerWrapper'),
				$tabsWrapper = $tabs.parents('#stTabswrapper').width(870);//$tabs.outerWidth(true) replace with widht of the tab
				//Fixing safari bug
				if($.browser.safari)
				{
					$tabsWrapper.width(870+6);//$tabs.width() replace with 870 for project requirement
				}
			...
			var arrowsTopMargin = (parseInt(parseInt($tabsNav.innerHeight(true)/2)+5)),
					...
					.append(
						$('<span/>')
							...
							.click(function(){
								...
								if($.browser.msie!=undefined && $.browser.version=="6.0"){
									var navWidth = $tabsNav.find('li').eq(prevIndex).position().left;
									if(navWidth==null){
										navWidth=0;
									}
									if(navWidth<800){										
						...
						$('<span/>')
							...
							.click(function(){
								...
								if($.browser.msie!=undefined && $.browser.version=="6.0"){
									var navWidth;									
									if($tabsNav.find('li').eq(nextIndex).position()==null){
										navWidth=$tabsNav.find('li').eq(nextIndex-1).position().left;
									}else{
										navWidth=$tabsNav.find('li').eq(nextIndex).position().left;
									}									
									if(navWidth>800){										
										$("#tabs").scrollLeft(0+navWidth-800);
									}								
								}
								//Now select the tab
			...
			.bind('addCloseButton',function(){
				...
				$(this).find('.ui-tabs-nav li').each(function(){
					...
					var closeTopMargin = parseInt(parseInt($tabsNav.find('li:first').innerHeight()/2,10)-7);
			...
			.bind('scrollToTab',function(event,$tabToScrollTo,clickedFrom,hiddenOnSide){
				...
				offsetLeft = -($tabs.width()-($tabToScrollTo.outerWidth(true)+navWidth+parseInt($tabsNav.find('li:last').css('margin-right'),0)));
			...
			//.trigger('navHandler') //cnu chages--> this method add style display as none then it won't be the display in tabs next and previous buttons 
			.trigger('navEnabler');
